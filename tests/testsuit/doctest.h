...
#else
        template <class T>
        inline typename remove_reference<T>::type& forward(typename remove_reference<T>::type& t) DOCTEST_NOEXCEPT
        {
            return t;
        }
#endif
...
#ifdef DOCTEST_CONFIG_WITH_RVALUE_REFERENCES
#define DOCTEST_RV_REF_WRAP(type) type&&
#else
#define DOCTEST_RV_REF_WRAP(type) const type&
#endif

#define DOCTEST_DO_BINARY_EXPRESSION_COMPARISON(op, op_str, op_macro)                              \
    template <typename R>                                                                          \
    DOCTEST_NOINLINE SFINAE_OP(Result,op) operator op(DOCTEST_RV_REF_WRAP(R) rhs) {          \
